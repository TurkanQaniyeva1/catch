...
try {
    const userWithQ = users.find(user => user.firstName.toLowerCase().endsWith('q'));
    console.log(userWithQ.firstName);
} catch (error) {
    console.log(error.message, "'q' ile biten ad yoxdur");
}
...
const persondetailid = (id) => {
    try {
        let findpersonid = users.find((item) => item.id == id);
        console.log(findpersonid);
        
        if (!findpersonid) {
            throw new Error("Bu id aid isdifadeci tapilmadi: " + id);  
        }
    } catch (error) {
        alert(error);
    }
...
const personadress = (id) => {
    try {
        let findperson = users.find((item) => item.id == id);
        console.log(findperson);

        if (!findperson.address) {
            throw new Error("  bu id li isdifadecinin adress melumati movcud deyil: " + id); 
        }
...
const persondetail = (id) => {
    try {
        ...
        if (!findperson.weight || !findperson.height) {
            throw new Error("Boy ve kilo melumati eksikdi: " + id);
        }
    } catch (error) {
        alert(error);
    }
